...
public abstract class BattleLoc extends Location{
    ...
    public boolean combat(int value){
        ...
        for (int i = 1; i <= value; i++) {
            if(this.getObstacle().getName().equals("Yılan")){
                //yılanın id'si 6, her seferde random damage alacak yılan
                System.out.println("***********------------------*************--------------***********----------");
    ...
    private void snakeAward() {
        ...
        else { //kazanacağı ödülü belirleme kısmı
            int awd=random.nextInt(1,101);
            if(awardRand<=60){  //%15 ihtimal silah

                if(awd<=20){ //tüfek kazanma şansı
                    this.getPlayer().getInventory().setWeapon(new Rifle());
                }
                else if(awd<=50){ //kılıç kazanma şansı
                    this.getPlayer().getInventory().setWeapon(new Sword());
                }
                else { //kalan ihtimallerde tabanca kazanıyor.
    ...
    public void setNumObs(int id) {
        if(id<6){ //yılan harici baska bir canavar ise 1-3
            this.numObs = random.nextInt(1,4);
        }else { //yılan canavarı secilir ise 1-5
